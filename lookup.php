<html>
    ...
    try {
        $tag = $_GET['tag'];
        $sql = 'SELECT posts.id, posts.titel, posts.auteur_id, posts.datum, posts.inhoud, posts.likes FROM posts 
        JOIN posts_tags ON posts.id = posts_tags.post_id JOIN tags ON posts_tags.tag_id = tags.id WHERE tags.titel = :tag';
        ...
        <?php foreach ($posts as $post) {
            ...
            <div class="post">
                <div class="header">
                    ...
                    <button type="submit" value="<?php echo $post->id; ?>" name="like"><?php echo $post->likes; ?> likes</button>
                    </form>
                    </span>
                </div>
                <span class="details">Geschreven op: <?php echo $post->datum . ' door <b>' . $post->auteur_id . '</b>';  ?></span>
